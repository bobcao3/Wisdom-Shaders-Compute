...
void main()
{
    ...
    vec3 s = imageLoad(colorimg3, iuv * 2).rgb;
    s += imageLoad(colorimg3, iuv * 2 + ivec2(0, 1)).rgb;
    s += imageLoad(colorimg3, iuv * 2 + ivec2(1, 1)).rgb;
    s += imageLoad(colorimg3, iuv * 2 + ivec2(1, 0)).rgb;

    s *= 0.25;

    imageStore(colorimg3, ivec2(h_offset, 0) + iuv, vec4(s, 0.0));
    ...
    for (int lod = 1; lod <= 4; lod++)
    {
        ...
        imageStore(colorimg3, (iuv >> lod) + ivec2(h_offset, 0), vec4(s, 0.0));
    }
